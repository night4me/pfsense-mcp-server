"""Live integration test for get_freeradius_eap. GENERATED PROPOSAL — review before use.
...
from pfsense_mcp.models.free_radius_eap import FreeRadiusEap
...
def test_get_freeradius_eap_live_structure_only():
    config = load_config()
    api_key = load_api_key(config)
    transport, client = build_pfsense_client(config, api_key)
    try:
        result = client.get_freeradius_eap()

        assert isinstance(result, FreeRadiusEap)
    finally:
        transport.close()
