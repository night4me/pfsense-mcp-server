"""Live integration test for get_firewall_advanced_settings. GENERATED PROPOSAL — review before use.
...
from pfsense_mcp.models.firewall_advanced_settings import FirewallAdvancedSettings
...
def test_get_firewall_advanced_settings_live_structure_only():
    config = load_config()
    api_key = load_api_key(config)
    transport, client = build_pfsense_client(config, api_key)
    try:
        result = client.get_firewall_advanced_settings()

        assert isinstance(result, FirewallAdvancedSettings)
    finally:
        transport.close()
