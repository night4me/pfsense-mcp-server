...
from pfsense_mcp.models.dns_resolver_settings import DnsResolverSettings
...
def test_get_dns_resolver_settings_live_structure_only():
    config = load_config()
    api_key = load_api_key(config)
    transport, client = build_pfsense_client(config, api_key)
    try:
        result = client.get_dns_resolver_settings()

        assert isinstance(result, DnsResolverSettings)
    finally:
        transport.close()
