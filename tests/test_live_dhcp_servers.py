"""Live integration test for get_dhcp_servers. GENERATED PROPOSAL — review before use.
...
def test_get_dhcp_servers_live_structure_only():
    config = load_config()
    api_key = load_api_key(config)
    transport, client = build_pfsense_client(config, api_key)
    try:
        result = client.get_dhcp_servers(limit=5)
