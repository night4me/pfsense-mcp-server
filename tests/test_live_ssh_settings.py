"""Live integration test for get_ssh_settings. GENERATED PROPOSAL — review before use.
...
from pfsense_mcp.models.ssh_settings import SshSettings
...
def test_get_ssh_settings_live_structure_only():
    config = load_config()
    api_key = load_api_key(config)
    transport, client = build_pfsense_client(config, api_key)
    try:
        result = client.get_ssh_settings()

        assert isinstance(result, SshSettings)
    finally:
        transport.close()
