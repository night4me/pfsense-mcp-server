"""Live integration test for get_diagnostics_tables. GENERATED PROPOSAL — review before use.
...
def test_get_diagnostics_tables_live_structure_only():
    config = load_config()
    api_key = load_api_key(config)
    transport, client = build_pfsense_client(config, api_key)
    try:
        result = client.get_diagnostics_tables(limit=5)
