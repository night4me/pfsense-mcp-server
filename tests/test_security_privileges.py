...
is a real, trimmed subset (53 paths -- the current 52 READ + 1 WRITE
...
plus nine entries carried over verbatim from the live production/LAB
...
virtual IPs, certificate authorities, IPsec SA/child-SA status) -- not
...
def test_read_profile_requirements_has_53_entries_with_exactly_one_local_only():
...
def test_read_profile_resolves_to_the_currently_verified_52_privileges(live_schema):
...
def test_write_protected_profile_resolves_to_the_currently_verified_53_privileges(live_schema):
    resolved = resolve_profile_privileges(live_schema, write_protected_profile_requirements())
    assert all(r.ok for r in resolved), [r.error for r in resolved if not r.ok]
    privileges = distinct_ok_privileges(resolved)
    assert len(privileges) == 53
