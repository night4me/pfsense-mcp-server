"""Live integration test for get_system_hasync. GENERATED PROPOSAL — review before use.
...
def test_get_system_hasync_live_structure_only():
    config = load_config()
    api_key = load_api_key(config)
    transport, client = build_pfsense_client(config, api_key)
    try:
        result = client.get_system_hasync()

        assert isinstance(result, SystemHaSync)
        assert result.username is None
        assert result.pfhostid is None
    finally:
        transport.close()
