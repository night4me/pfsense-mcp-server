"""Live integration test for get_acme_settings. GENERATED PROPOSAL — review before use.
...
from pfsense_mcp.models.acme_settings import AcmeSettings
...
def test_get_acme_settings_live_structure_only():
    config = load_config()
    api_key = load_api_key(config)
    transport, client = build_pfsense_client(config, api_key)
    try:
        result = client.get_acme_settings()

        assert isinstance(result, AcmeSettings)
    finally:
        transport.close()
