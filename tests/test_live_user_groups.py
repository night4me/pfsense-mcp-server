"""Live integration test for get_user_groups. GENERATED PROPOSAL — review before use.
...
def test_get_user_groups_live_structure_only():
    config = load_config()
    api_key = load_api_key(config)
    transport, client = build_pfsense_client(config, api_key)
    try:
        result = client.get_user_groups(limit=5)
