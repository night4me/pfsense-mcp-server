"""Live integration test for get_carp_status. GENERATED PROPOSAL — review before use.
...
def test_get_carp_status_live_structure_only():
    config = load_config()
    api_key = load_api_key(config)
    transport, client = build_pfsense_client(config, api_key)
    try:
        result = client.get_carp_status()

        assert isinstance(result, CarpStatus)
    finally:
        transport.close()
