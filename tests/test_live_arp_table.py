"""Live integration test for get_arp_table. GENERATED PROPOSAL — review before use.
...
def test_get_arp_table_live_structure_only():
    config = load_config()
    api_key = load_api_key(config)
    transport, client = build_pfsense_client(config, api_key)
    try:
        result = client.get_arp_table(limit=5)
