"""Live integration test for get_email_notification_settings. GENERATED PROPOSAL — review before use.
...
def test_get_email_notification_settings_live_structure_only():
    config = load_config()
    api_key = load_api_key(config)
    transport, client = build_pfsense_client(config, api_key)
    try:
        result = client.get_email_notification_settings()
