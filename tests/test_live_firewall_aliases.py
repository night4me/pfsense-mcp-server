"""Live integration test for get_firewall_aliases. GENERATED PROPOSAL — review before use.
...
def test_get_firewall_aliases_live_structure_only():
    config = load_config()
    api_key = load_api_key(config)
    transport, client = build_pfsense_client(config, api_key)
    try:
        result = client.get_firewall_aliases(limit=5)
