...
def _build_parser() -> argparse.ArgumentParser:
    ...
    write_client_config_parser = setup_subparsers.add_parser(
        ...
        epilog=(
            ...
            "target path (or its parent directory) is unsafe (a symbolic link, not a regular file/"
            "directory, or not owned by the current user) -- a missing parent directory is created "
            "automatically (owner-only permissions) when safe to do so; this exit code is for when it "
            "is not. 7 the file was written but did not read back exactly as intended -- the original "
