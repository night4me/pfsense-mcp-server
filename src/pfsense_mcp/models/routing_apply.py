"""Model for the RoutingApply capability endpoint.

Field types were derived from the pinned v2.10 OpenAPI schema's
`RoutingApply` component (already-captured evidence, not a new live
...
class RoutingApply(BaseModel):
    applied: bool | None

    @classmethod
    def from_api(cls, data: dict[str, Any]) -> "RoutingApply":
