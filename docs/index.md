...
**Current production contract: 84 READ tools. 0 WRITE tools.** That split
...
- **API reference** — the full MCP tool catalog this server currently
  registers (84 READ tools, zero WRITE tools).
...
The production server is READ-only: 84 tools, zero WRITE tools, an empty
