...
| [017](ADR-017-official-guidance-layer.md) | Official pfSense/Netgate documentation guidance layer | Accepted — as of 2026-08-22, one narrowly-scoped MCP guidance tool (`pfsense_get_official_guidance`) consumes it in production; live retrieval remains deferred |
| [018](ADR-018-version-aware-guidance-resolution.md) | Version-aware Official Guidance resolution | Accepted — architecture and trust boundaries; as of 2026-08-22, `resolve_appliance_identity()` and version-aware resolution are implemented and wired via `pfsense_get_official_guidance`; live retrieval, Tier 1/WRITE/Phase 5 activation remain separately gated and not authorized |
...
| [037](ADR-037-write-programme-architecture.md) | WRITE Programme Architecture — capability families, existence-transition executor, risk scaling | Accepted (2026-09-04, owner) — architecture only, no new WRITE capability authorized; re-derives the full 451-operation mutation inventory, classifies every operation (verified three times: an owner acceptance re-review, a narrow amendment pass correcting its findings, then a clean acceptance review confirming 26 stated criteria), and identifies that 148 operations (32.8%, create/delete-shaped) require a not-yet-designed second sealed executor before implementation |
| [038](ADR-038-expired-prepared-retry-semantics.md) | Expired/terminal-PREPARED retry semantics — active-scoped idempotency uniqueness, `expire_prepared()`, composition-layer stale-artifact refusal | Accepted (2026-09-05, owner). Implemented — `idempotency_key` uniqueness narrows from whole-table to blocking-`RecoveryState` scope only; a FAILED/ROLLED_BACK/EXPIRED historical contract no longer permanently blocks a fresh, independently-authorized retry, but never authorizes one on its own. `expire_prepared()` remains a local-only store primitive with no CLI/MCP/operator exposure. |
