# ADR-032: Nexus READ transport architecture (design only, Phase E)

- **Status:** Design/specification only. **Not implemented.** No HTTP
  client, JWT handling, or device-routing code exists in this repository
  as a result of this ADR. Owner-authorized explicitly as "NOT
  authorization to build the full transport yet" — the deliverable is a
  design precise enough that a future, separately authorized Phase F
  could implement it without further research.
