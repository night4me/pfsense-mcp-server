...
- The default profile registers 95 READ tools + 1 guidance tool (96
  total; a `v0.9.0` candidate adds a second guidance tool but is not
  yet published — see `CHANGELOG.md`), 0 WRITE tools. Selecting
  `write_protected` is a separate, deliberate opt-in — see [the setup
