...
| Missing guidance for a valid capability | Explicit empty-result path when the capability has no registry entry at all (I6, `NO_OFFICIAL_GUIDANCE_FOUND`), never fabricated | None — this is accepted, by-design behavior, not a gap to close |
| Ambiguous guidance (ambiguous version/edition match) | A *registered* entry is returned labeled with its true `ApplicabilityState` (`VERSION_UNCONFIRMED`/`EDITION_MISMATCH`/`STALE`) rather than silently omitted (I6, revised 2026-08-09 — inclusion-with-state, an owner-authorized policy change from the original exclude-only design) — still never a low-confidence guess presented as `APPLICABLE` | None beyond what the revised I6 already requires |
