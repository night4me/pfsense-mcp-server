...
| Wrong pfSense edition/version guidance | Edition/version fail-closed to empty result on any ambiguity (I6); no guess-and-serve path exists | No edition-discriminator READ field exists (`SystemVersion` has none); `ADR-018` (**Accepted** 2026-08-09, architecture and trust boundaries only) designs a non-fragile edition inference from `SystemVersion.base`'s documented version-numbering-scheme shape, independently red-teamed and separately final-acceptance-reviewed (`reports-ai/reviews/ADR_018_RED_TEAM.md`, `reports-ai/reviews/ADR_018_ACCEPTANCE_REVIEW.md`) — remains a design, not an active mitigation, until each piece is separately implemented and activated per ADR-018's own "Activation requirements" |
