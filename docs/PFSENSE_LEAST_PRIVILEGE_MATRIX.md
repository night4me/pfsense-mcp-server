...
exists for them. **None of the 41 endpoints this project's 42 READ
tools use require `page-all`**, confirmed by direct inspection of every
matching `Endpoints/*.inc` file at the pinned tag. This must be
...
mismatches, zero endpoints missing from the live schema.
...
## READ privilege matrix (42 tools)
...
**41 distinct privileges, one per tool, zero sharing between tools** —
confirmed programmatically (`len(set(privileges)) == 41`). A least-privilege
READ-only identity holding exactly these 41 (never `page-all`) can serve
every one of this project's 42 registered READ tools.
...
completeness, since a future decision to expose it as tool #43 would
need this privilege:
...
The 4 WRITE privileges are a strict subset of the 41 READ privileges
...
identity therefore needs exactly **42 distinct privileges**: the 41 READ
