...
exists for them. **None of the 48 endpoints this project's 49 READ
...
interface-VLAN/static-route/group, firewall-schedule, and REST-API-
version endpoints, each of which offers a narrow alternative alongside
...
pair, and the interface-group/firewall-schedule/REST-API-version trio,
both added 2026-08-20, were cross-checked against a live schema
...
match, zero mismatches (48 of 48 now, including all five).

## READ privilege matrix (49 tools)
...
**48 distinct privileges, one per tool, zero sharing between tools** —
confirmed programmatically (`len(set(privileges)) == 48`). A least-privilege
READ-only identity holding exactly these 48 (never `page-all`) can serve
every one of this project's 49 registered READ tools.
...
completeness, since a future decision to expose it as a 50th tool would
need this privilege:
...
The 4 WRITE privileges are a strict subset of the 48 READ privileges
...
identity therefore needs exactly **49 distinct privileges**: the 48 READ
