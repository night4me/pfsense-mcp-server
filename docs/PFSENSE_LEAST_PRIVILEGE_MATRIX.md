...
exists for them. **None of the 50 endpoints this project's 51 READ
...
outbound-NAT/1:1-NAT mapping endpoints and, as of 2026-08-20, the
interface-VLAN/static-route/group, firewall-schedule/virtual-IP,
REST-API-version, and certificate-authority endpoints, each of which
...
pair, the interface-group/firewall-schedule/REST-API-version trio, and
the firewall-virtual-IP/certificate-authority pair, all added
2026-08-20, were cross-checked against a live schema freshly fetched
from the **LAB** appliance (`pfsense-test.lab.invalid`) during
owner-authorized LAB READ verification passes — also an exact match,
zero mismatches (50 of 50 now, including all seven).

## READ privilege matrix (51 tools)
...
**50 distinct privileges, one per tool, zero sharing between tools** —
confirmed programmatically (`len(set(privileges)) == 50`). A least-privilege
READ-only identity holding exactly these 50 (never `page-all`) can serve
every one of this project's 51 registered READ tools.
...
completeness, since a future decision to expose it as a 52nd tool would
need this privilege:
...
The 4 WRITE privileges are a strict subset of the 50 READ privileges
...
identity therefore needs exactly **51 distinct privileges**: the 50 READ
