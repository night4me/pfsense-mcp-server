...
exists for them. **None of the 45 endpoints this project's 46 READ
...
interface-VLAN/static-route endpoints, each of which offers a narrow
...
pair added 2026-08-20 was cross-checked against a live schema freshly
fetched from the **LAB** appliance (`pfsense-test.lab.invalid`) during
an owner-authorized LAB READ verification — also an exact match, zero
mismatches (45 of 45 now, including these two).

## READ privilege matrix (46 tools)
...
**45 distinct privileges, one per tool, zero sharing between tools** —
confirmed programmatically (`len(set(privileges)) == 45`). A least-privilege
READ-only identity holding exactly these 45 (never `page-all`) can serve
every one of this project's 46 registered READ tools.
...
completeness, since a future decision to expose it as a 47th tool would
need this privilege:
...
The 4 WRITE privileges are a strict subset of the 45 READ privileges
...
identity therefore needs exactly **46 distinct privileges**: the 45 READ
