...
exists for them. **None of the 43 endpoints this project's 44 READ
tools use require `page-all`**, confirmed by direct inspection of every
matching `Endpoints/*.inc` file at the pinned tag (including, as of
2026-08-20, the two outbound-NAT/1:1-NAT mapping endpoints, each of
which offers a narrow alternative alongside `page-all`). This must be
...
source this document reasons about.

## READ privilege matrix (44 tools)
...
**43 distinct privileges, one per tool, zero sharing between tools** —
confirmed programmatically (`len(set(privileges)) == 43`). A least-privilege
READ-only identity holding exactly these 43 (never `page-all`) can serve
every one of this project's 44 registered READ tools.
...
completeness, since a future decision to expose it as tool #45 would
need this privilege:
...
The 4 WRITE privileges are a strict subset of the 43 READ privileges
...
identity therefore needs exactly **44 distinct privileges**: the 43 READ
